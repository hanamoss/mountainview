(function($) {
	...
	if ($('#mg-map').length) {
		...
		map.addMarker({
			...
			title: 'Map',
			infoWindow: {
				content: '<strong>Crowd Valley</strong><br>One Market Plaza, Spear Tower <br> San Francisco, CA 94105 <br>UNITED STATES'
			}
		});
	}
	...
	$.validate({
		form: '#form_add_debt_offering',
		modules : 'security'
	});
	$.validate({
		form: '#form_add_equity_offering',
		modules : 'security'
	});
	$.validate({
		form: '#form_add_org_real_estate',
		modules : 'security'
	});
	$.validate({
		form: '#form_add_org_private_company',
		modules : 'security'
	});
